...
// Holds streams of data to override mesh's buffers without making the mesh unique.
[ExecuteInEditMode]
public class VertexStreamComponent : MonoBehaviour {
	[HideInInspector]
	[SerializeField]
	...
	void EnforceOriginalMeshHasColors (Mesh stream) {
		...
		if (stream != null && stream.colors.Length > 0 && (origColors == null || origColors.Length == 0)) {
			UnityEngine.Profiling.Profiler.BeginSample ("Set mesh colors");
	...
	public Mesh Apply (bool markNoLongerReadable = true) {
		...
		if (mr != null && mf != null) {
			UnityEngine.Profiling.Profiler.BeginSample ("Creating additionalVertexStream Data");
			...
			if (stream == null || vertexCount != stream.vertexCount) {
				UnityEngine.Profiling.Profiler.BeginSample ("Create Mesh stream");
				if (stream != null) {
			...
			UnityEngine.Profiling.Profiler.BeginSample ("Generating Mesh");
			...
			UnityEngine.Profiling.Profiler.BeginSample ("Uploading mesh data");
