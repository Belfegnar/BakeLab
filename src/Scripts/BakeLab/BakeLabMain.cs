...
namespace Belfegnar.BakeLab {
	...
	public class BakeLabMain : MonoBehaviour {
		...
		public int Bake () {
...
#if DEBUG
#if UNITY_EDITOR
			UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Prepare scene ...", 0.5f);
#endif
			...
			Scene scene = new Belfegnar.BakeLab.Scene ();
...
#if DEBUG
				Debug.Log ("Failed to prepare scene, exiting");
#if UNITY_EDITOR
...
#if UNITY_EDITOR
			UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Generate sample points ...", 0.5f);
#endif
			...
			UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Distribute samples ...", 0.5f);
#endif
...
#if DEBUG && UNITY_EDITOR
			UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Sample instances ...", 0.5f);
#endif
...
#if UNITY_EDITOR
			UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Compute Signal ...", 0.5f);
#endif
			...
			switch (config.signalType) {
				case SignalType.AO:
					...
					Belfegnar.BakeLab.Baking.ComputeAO (scene, samples, config.numRays, sceneOffset, sceneMaxdistance, config.useCpu, floatValues, parallel);
					break;

				case SignalType.Radiance:
					floatValues = new float[totalSamples];
					floatValuesList.Add (floatValues);
					Belfegnar.BakeLab.Baking.ComputeRadiance (scene, samples, config.numRays, sceneOffset, sceneMaxdistance, config.useCpu, floatValues);
					break;
				...
				case SignalType.LightDirTangent:
					...
					for (int si = 0; si < numSignals; si++) {
						floatValues = new float[totalSamples];
						floatValuesList.Add (floatValues);
						Belfegnar.BakeLab.Baking.ComputeTangentSpaceLightDirection (scene, samples, config.numRays, sceneOffset, sceneMaxdistance, config.useCpu, floatValues, si);
					}
					break;
				...
				case SignalType.Irradiance:
					...
					Belfegnar.BakeLab.Baking.ComputeIrradiance (scene, samples, config.packStyle, config.numRays, sceneOffset, sceneMaxdistance, config.useCpu, floatValuesList);
					break;
			}
#if DEBUG
			Debug.Log (timer.Elapsed.TotalMilliseconds);
...
#if DEBUG && UNITY_EDITOR
			UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Map Signal to vertices ...", 0);
#endif
...
#if DEBUG && UNITY_EDITOR
						if (i % 1000 == 0 && UnityEditor.EditorUtility.DisplayCancelableProgressBar ("Belfegnar.BakeLab", "Load scene ...", i * 1f / li))
							return null;
#endif
						v1 = mesh.vertices[i];
