		...
		public static class Irradiance {
			...
			public static void BakeIrradiance (
				...
				for (int i = 0; i < numSamples; i++) {
					...
					if (i % 1000 == 0 && UnityEditor.EditorUtility.DisplayCancelableProgressBar ("Belfegnar.Baking", string.Format ("Calc irradiance values ...    {0:D}/{1:D}", i, numSamples), i * 1f / numSamples))
						return;
#endif
					//sh = probes[i];
