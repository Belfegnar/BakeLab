...
using UnityEngine;
using EdgeMap = System.Collections.Generic.Dictionary<Belfegnar.BakeLab.Vector2i, Belfegnar.BakeLab.Butterfly>;
using TripletMap = System.Collections.Generic.Dictionary<Belfegnar.BakeLab.Vector2i, System.Double>;
...
namespace Belfegnar.BakeLab {
	...
	public static class Filter {
		...
		#region Least_Squares_Filtering
		...
		static void FilterMeshLeastSquares (
			...
			if (!parallel)
				UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Building mass_matrix ...", 0);
#endif
...
#if DEBUG && UNITY_EDITOR
			if (!parallel)
				UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Decompose A ...", 0.5f);
#endif
...
#if DEBUG && UNITY_EDITOR
			if (!parallel)
				UnityEditor.EditorUtility.DisplayProgressBar ("Belfegnar.Baking", "Solve ...", 0.5f);
#endif
			solveTimer.Start ();
