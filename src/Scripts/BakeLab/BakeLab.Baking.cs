...
namespace Belfegnar.BakeLab {
	public static partial class Baking {
		...
		public static void ComputeAO (
			...
			if (parallel)
				BakeAOParallel (scene, samples, raysPerSample, sceneOffset, sceneMaxdistance, cpuMode, aoValues);
			else
		...
		static void BakeAOParallel (
			...
			var results = new NativeArray<RaycastHit> (numSamples * raysPerSample, Allocator.Temp);
			var commands = new NativeArray<RaycastCommand> (numSamples * raysPerSample, Allocator.Temp);
			for (int i = 0; i < numSamples; i++) {
#if DEBUG && UNITY_EDITOR
				if (i % 1000 == 0 && UnityEditor.EditorUtility.DisplayCancelableProgressBar ("Belfegnar.Baking", string.Format ("Calc rays ...    {0:D}/{1:D}", i, numSamples), i * 1f / numSamples))
					return;
#endif
...
#if false
					ray.from = samplePos + sampleNorm * sceneOffset + sceneMaxdistance * dir;
					ray.direction = -dir;
					ray.distance = sceneMaxdistance - sceneOffset;
					ray.maxHits = 1;

					commands[i * raysPerSample + s] = ray;
#else
					ray.from = samplePos + sampleNorm * sceneOffset;
					ray.direction = dir;
					ray.distance = sceneMaxdistance;
					ray.maxHits = 1;
			...
			JobHandle handle = RaycastCommand.ScheduleBatch (commands, results, 1000, default (JobHandle));
#if DEBUG && UNITY_EDITOR
			if (UnityEditor.EditorUtility.DisplayCancelableProgressBar ("Belfegnar.Baking", string.Format ("Wait for the batch processing job to complete..."), 0.5f))
				return;
#endif
			...
			for (int i = 0; i < numSamples; i++) {
				...
				for (int s = 0; s < raysPerSample; s++) {
					...
					if (results[i * raysPerSample + s].collider != null) {
						hits += 1f;
					}
				}
