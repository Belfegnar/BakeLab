...
namespace CSparse.Single.Factorization
{
    ...
    /// by Tim Davis.
    /// </remarks>
    public class SparseLU : ISparseFactorization<float>
    {
        SymbolicFactorization S;
        CompressedColumnStorage<float> L, U;
        ...
        public static SparseLU Create(CompressedColumnStorage<float> A, ColumnOrdering order,
            float tol)
        {
        ...
        /// <param name="tol">Partial pivoting tolerance (form 0.0 to 1.0).</param>
        public static SparseLU Create(CompressedColumnStorage<float> A, int[] p, float tol)
        {
            ...
            IProgress progress)
        {
            ...
            tol = Math.Min(Math.Max(tol, 0.0f), 1.0f);
        ...
        public void Solve(float[] input, float[] result)
        {
        ...
        /// <param name="input">The right hand side vector, <c>b</c>.</param>
        /// <param name="result">The left hand side vector, <c>x</c>.</param>
        public void SolveTranspose(float[] input, float[] result)
        {
            if (input == null) throw new ArgumentNullException("input");
        ...
        /// </summary>
        private void Factorize(CompressedColumnStorage<float> A, float tol, IProgress progress)
        {
            int[] q = S.q;
            ...
            this.L = CompressedColumnStorage<float>.Create(n, n, lnz);
            this.U = CompressedColumnStorage<float>.Create(n, n, unz);
            this.pinv = new int[n];
        ...
        /// <param name="A"></param>
        /// <param name="p">Permutation.</param>
        private void SymbolicAnalysis(CompressedColumnStorage<float> A, int[] p)
        {
        ...
        private int SolveSp(CompressedColumnStorage<float> G, CompressedColumnStorage<float> B,
            int k, int[] xi, float[] x, int[] pinv, bool lo)
        {
